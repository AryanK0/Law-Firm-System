...
exports.ensureCaseExists = ensureCaseExists;
exports.ensureCaseAccess = ensureCaseAccess;
exports.listCases = listCases;
exports.getCaseDetail = getCaseDetail;
exports.getCaseTeam = getCaseTeam;
exports.getCaseStatusHistory = getCaseStatusHistory;
exports.getCaseBilling = getCaseBilling;
...
`;
async function checkCaseAccess(employeeId, caseId) {
    const access = await (0, db_1.queryOne)("SELECT check_access(?, ?) AS has_access", [employeeId, caseId]);
    return Boolean(access?.has_access);
}
async function ensureCaseExists(caseId) {
    const row = await (0, db_1.queryOne)("SELECT case_id FROM Cases WHERE case_id = ?", [caseId]);
    if (!row) {
        throw new http_1.ApiError(404, "Case not found.");
    }
}
async function ensureCaseAccess(employeeId, caseId) {
    await ensureCaseExists(caseId);
    if (!(await checkCaseAccess(employeeId, caseId))) {
        throw new http_1.ApiError(403, "You do not have access to this case.");
    }
}
async function listCases(employeeId, filters = {}) {
    ...
    return (0, db_1.queryRows)(`${CASE_SUMMARY_SQL}
     WHERE ${conditions.join(" AND ")}
     ORDER BY c.case_id DESC`, params);
}
async function getCaseDetail(caseId) {
    const row = await (0, db_1.queryOne)(`${CASE_DETAIL_SQL} WHERE c.case_id = ?`, [caseId]);
    if (!row) {
        throw new http_1.ApiError(404, "Case not found.");
    }
    const hearings = await (0, db_1.queryRows)(`
    SELECT
      h.hearing_id,
      h.date,
      h.notes,
...
async function getCaseTeam(caseId) {
    const team = await (0, db_1.queryRows)(`
    SELECT
      ct.employee_id,
      ct.role_in_case,
      ct.assigned_by,
...
async function getCaseStatusHistory(caseId) {
    const history = await (0, db_1.queryRows)(`
    SELECT
      h.history_id,
      h.old_status,
      h.new_status,
...
async function getCaseBilling(caseId) {
    const entries = await (0, db_1.queryRows)(`
    SELECT
      b.bill_id,
      b.amount,
      b.status,
    ...
    WHERE case_id = ?
    `, [caseId]);
    const hours = await (0, db_1.queryOne)(`
    SELECT
...
async function createCase(payload) {
    const created = await (0, db_1.callProcedureOne)("create_case_full", [
    ...
    if (!created?.case_id) {
        throw new http_1.ApiError(400, "Case creation did not return a new id.");
    }
