...
async function getAnalytics() {
    ...
    const [total_cases, open_cases, total_tickets, breached_tickets, documents] = await Promise.all([
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Cases"),
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Cases WHERE status <> 'Closed'"),
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Ticket"),
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Ticket WHERE breach_flag = TRUE"),
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Document"),
    ]);
    return {
        summary: {
...
async function getOverview() {
    const [active_people, open_matters, upcoming_hearings_total, open_tickets, active_clients, tracked_revenue, pending_bills, sla_risk, featured_people, role_access, priority_matters, upcoming_hearings, recent_documents, support_watch, department_coverage, client_portfolio, recent_interactions, billing_watch,] = await Promise.all([
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Employee WHERE status = 'Active'"),
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Cases WHERE status <> 'Closed'"),
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Hearing WHERE date >= CURDATE()"),
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Ticket WHERE status <> 'Resolved'"),
        (0, db_1.queryOne)("SELECT COUNT(DISTINCT client_id) AS total FROM Cases WHERE status <> 'Closed'"),
        (0, db_1.queryOne)("SELECT COALESCE(SUM(amount), 0) AS total FROM Billing"),
        (0, db_1.queryOne)("SELECT COUNT(*) AS total FROM Billing WHERE status = 'Pending'"),
        (0, db_1.queryOne)(`
